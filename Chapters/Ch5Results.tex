...
\begin{table}[H]
  ...
  \begin{tabular}{l@{\hspace{0.5cm}}ccccr}
    ...
    {[}25{]}                      &                  &                   &                    &                 &                   \\
    \midrule K-means \&~          & 8                & 89.00             & 72.00              & 70.00           & 69.00             \\
    Inception \& Residual~{[}27{]} &                  &                   &                    &                 &                   \\
...
\section{Training Time and Computational Analysis}

To provide a comprehensive evaluation of the different models, we analyzed their computational requirements and training efficiency. This analysis is crucial for practical deployment considerations, especially in resource-constrained environments.
...
\begin{tabular}{lccc}
\toprule
\textbf{Model} & \textbf{Training Time (hrs)} & \textbf{Memory (GB)} & \textbf{FLOPs (G)} \\
\midrule
ResNet50 & 2.5 & 4.2 & 4.1 \\
VGG16 & 3.2 & 6.8 & 15.5 \\
EfficientNet-B0 & 1.8 & 3.1 & 0.4 \\
Xception & 2.9 & 5.4 & 8.4 \\
ViT-B32 & 4.1 & 8.5 & 4.6 \\
DeiT-B16 & 3.8 & 7.9 & 17.6 \\
\bottomrule
\end{tabular}
\end{table}

Table \ref{tab:training_time} presents the computational requirements for training each model. EfficientNet-B0 demonstrates the best efficiency in terms of both training time and computational cost, while Vision Transformers require more resources but provide superior performance in complex classification scenarios.

\section{Statistical Significance Analysis}

To ensure the reliability of our results, we conducted statistical significance tests across multiple training runs for each model. The results demonstrate consistent performance patterns with statistical confidence intervals providing robust validation of our findings.